...
struct trace_t
{
	...
	int contents;
	WORD pad1; // unsigned int dispFlags;
	...
	WORD pad2; //unsigned short worldSurfaceIndex;
...
class CGrenadePredictTraceFilter : ITraceFilter
{
public:
	virtual bool ShouldHitEntity(C_BaseEntity* pEntityHandle, int contentsMask)
	{
		if (!pEntityHandle)
			return true;
		if (pEntityHandle->GetClientClass()->m_ClassID == EClassIds::CCSPlayer)
			return false;
		ClientClass *clclass = pEntityHandle->GetClientClass();
		if (clclass)
		{
			if (!strcmp(clclass->m_pNetworkName, "projectile"))
				return false;
		}
		return pEntityHandle != pSkip;
	}
...
public:
	...
	virtual void TraceRay(const Ray_t &ray, unsigned int fMask, void *pTraceFilter, trace_t *pTrace) = 0;
	...
	void TraceRay2(const Ray_t &ray, unsigned int fMask, void* pTraceFilter, trace_t* pTrace)
	{
		typedef void(__thiscall* fnTraceRay)(void*, const Ray_t&, unsigned int, void*, trace_t*);
