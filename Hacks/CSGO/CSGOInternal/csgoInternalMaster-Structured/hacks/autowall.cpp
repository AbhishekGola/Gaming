...
bool Autowall::TraceToExit(Vector &end, trace_t *enter_trace, Vector start, Vector dir, trace_t *exit_trace)
{
	...
	while (distance <= 90.0f)
	{
		...
		auto point_contents = I::Trace->GetPointContents(end, MASK_SHOT_HULL | CONTENTS_HITBOX, NULL);
		...
		I::Trace->TraceRay(ray, MASK_SHOT, 0, exit_trace);
		...
		if (exit_trace->startsolid && exit_trace->surface.flags & SURF_HITBOX)
		{
			...
			I::Trace->TraceRay(ray, 0x600400B, &filter, exit_trace);
		...
		if (!(exit_trace->fraction < 1.0 || exit_trace->allsolid || exit_trace->startsolid) || exit_trace->startsolid)
		{
			...
			{
				if (enter_trace->m_pEntityHit && enter_trace->m_pEntityHit == I::EntList->GetClientEntity(0))
					return true;
			}
...
{
	CCSWeaponInfo* weaponInfo = G::ActiveWeaponInfo; //pWeapon->GetCSWpnData(); //I::WeaponSystem->GetWpnData(*pWeapon->GetItemDefinitionIndex());
	...
	while (data.penetrate_count > 0 && data.current_damage >= 1.0f)
	{
		...
		// data.enter_trace
		TraceLine(data.src, end, MASK_SHOT, G::LocalPlayer, &data.enter_trace);

		Ray_t ray;
		ray.Init(data.src, end + data.direction * 40.f);

		I::Trace->TraceRay(ray, MASK_SHOT, &data.filter, &data.enter_trace);

		TraceLine(data.src, end + data.direction * 40.f, MASK_SHOT, G::LocalPlayer, &data.enter_trace);
		...
		if (data.enter_trace.hitgroup <= HitGroups::HITGROUP_RIGHTLEG && data.enter_trace.hitgroup > HitGroups::HITGROUP_GENERIC)
		{
			...
			if (teamCheck && player->GetTeam() == G::LocalPlayer->GetTeam())
				return false;
...
float Autowall::GetDamage(const Vector& point, bool teamCheck, FireBulletData& fData)
{
	...
	if (SimulateFireBullet(G::ActiveWeapon, teamCheck, data))
